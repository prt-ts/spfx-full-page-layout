...
const OverflowBreadcrumbButton: React.FC<{ id: string; item: ButtonItem }> = (
  props
) => {
  const { item, id } = props;
...
const ControlledOverflowMenu = (
  props: PartitionBreadcrumbItems<ButtonItem>
) => {
...
const BreadcrumbControlledOverflowExample = () => {
  ...
  const {
    ...
    overflowItems,
    endDisplayedItems,
  }: PartitionBreadcrumbItems<ButtonItem> = partitionBreadcrumbItems({
    items: buttonItems,
    maxDisplayedItems: 4,
  });
